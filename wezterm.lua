...
------------------------------------------------------------
-- Startup session (kitty session.conf equivalent)
--   tab "argocd": cwd ~/dev/go/src/github.com/argoproj/argo-cd
--   tab "git":    cwd ~/dev/git
------------------------------------------------------------
wezterm.on("gui-startup", function(cmd)
  local home = wezterm.home_dir
  local _, _, window = wezterm.mux.spawn_window(cmd or {
    cwd = home .. "/dev/go/src/github.com/argoproj/argo-cd",
  })
  window:active_tab():set_title("argocd")

  local tab2, _, _ = window:spawn_tab({ cwd = home .. "/dev/git" })
  tab2:set_title("git")
end)
